...
export default function SocraticChat({ submissionId }: SocraticChatProps) {
  const { messages, sendMessage, status } = useChat({
    transport: new DefaultChatTransport({ api: '/api/chat', body: { submissionId } }),
  });
    ...
    <div className="flex flex-col h-[560px] w-full max-w-lg mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border-2 border-indigo-100">
      <div className="bg-[#4f46e5] text-white p-4 flex items-center space-x-3">
        ...
        <div>
          ...
          <div className="text-xs text-indigo-200 mt-0.5">{isLoading ? 'Thinking...' : 'Online'}</div>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50">
        ...
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {m.role === 'assistant' && (
              <div className="w-7 h-7 rounded-full bg-indigo-100 flex items-center justify-center text-sm mr-2 flex-shrink-0 mt-1">🤖</div>
            )}
            <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${m.role === 'user' ? 'bg-[#4f46e5] text-white rounded-br-none shadow-md' : 'bg-white text-slate-800 border border-indigo-100 rounded-bl-none shadow-sm'}`}>
              {m.parts.filter(p => p.type === 'text').map(p => p.type === 'text' ? p.text : '').join('')}
            </div>
          </div>
        ))}
        {isLoading && (
          <div className="flex justify-start">
            <div className="w-7 h-7 rounded-full bg-indigo-100 flex items-center justify-center text-sm mr-2 flex-shrink-0">🤖</div>
      ...
      <form onSubmit={handleSubmit} className="p-3 bg-white border-t border-indigo-100 flex items-center space-x-2">
        ...
        />
        <button type="submit" disabled={isLoading || !input.trim()} className="bg-[#4f46e5] text-white rounded-full w-10 h-10 flex items-center justify-center hover:bg-indigo-700 transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
